...
source(paste0(script_basename, "/utils/directory_utils.R"))
...
variable_metadata_generation = function(path_to_excel_directory,
  ...
  create_export_directories(path_to_json_directory = path_to_output_directory,
  ...
  for (data_set in stata_files) {
    ...
    variable_excel_dao <- VariableExcelDao$new(paste0(path_to_excel_directory, "/vimport_",
    ...
    cat(paste0("Assembling variables and exporting them to json for data set ",
      data_set_number, ".\n"))
    for (variable_name in variable_excel_dao$get_variable_names()) {
      ...
      variable$set_distribution(
        stata_data_set_dao$get_distribution(variable_name,
          variable$get_scale_level()$get_en(), variable$get_access_ways()))
      variable$to_json(paste0(path_to_output_directory, "/", data_set_number, "/",
        variable_name, ".json"))
    }
  }
}
